...
export function initDevTools(map) {
    ...
    style.textContent = `
        ...
        .dev-header { padding: 16px; border-bottom: 1px solid #f3f4f6; background: #f9fafb; border-radius: 12px 12px 0 0; }
        ...
        .merge-list { list-style: none; padding: 0; margin: 0; border: 1px solid #e5e7eb; border-radius: 6px; overflow: hidden; }
        .merge-item { padding: 8px 12px; border-bottom: 1px solid #f3f4f6; display: flex; justify-content: space-between; align-items: center; font-size: 13px; background: #fff; }
        .merge-item:last-child { border-bottom: none; }
        .merge-item:hover { background: #f9fafb; }
        .unmerge-btn { background: none; border: none; color: #ef4444; cursor: pointer; padding: 4px; border-radius: 4px; font-size: 16px; line-height: 1; }
        .unmerge-btn:hover { background: #fee2e2; }

        .action-btn { width: 100%; padding: 10px; border: none; border-radius: 8px; font-weight: 500; cursor: pointer; font-size: 14px; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 8px; }
        .btn-primary { background: #2563eb; color: white; }
        .btn-primary:hover { background: #1d4ed8; }
        .btn-outline { background: white; border: 1px solid #d1d5db; color: #374151; }
        .btn-outline:hover { background: #f3f4f6; border-color: #9ca3af; }
        .btn-save { background: #16a34a; color: white; margin-top: 16px; }
    ...
    panel.innerHTML = `
        ...
        <div class="dev-body">
            <div id="dev-content" style="display:none">
                ...
                <!-- Merge Section -->
                <div class="dev-section">
                    <div class="dev-section-title">Merged Stops</div>
                    <button id="btn-combine" class="action-btn btn-outline">
                        <span>🔗</span> Combine with another stop...
                    </button>
                    
                    <div style="display:flex; gap:8px; margin-top:8px">
                        <input type="text" id="manual-merge-id" class="dev-input" placeholder="Stop ID to merge (e.g. 1:123)" style="padding:4px 8px; font-size:12px">
                        <button id="btn-manual-merge" class="action-btn btn-outline" style="width:auto; padding:4px 12px; font-size:12px">Merge</button>
                    </div>

                    <ul id="merge-list" class="merge-list" style="margin-top:12px; display:none"></ul>
                </div>

                <div style="border-top:1px solid #eee; margin-top:16px; padding-top:16px; display:flex; gap:8px">
                     <button id="btn-save" class="action-btn btn-save" style="flex:1">Save to Disk (Server)</button>
                </div>
                ...
                <div style="text-align:center; font-size:10px; color:#9ca3af; margin-top:4px">
                    "Save to Disk" requires server restart.
                </div>
            </div>
    ...
    let isPicking = false;
    let mergeTargetId = null; // Store the ID we are merging INTO
    ...
    function setPickMode(active) {
        ...
        if (active) {
            ...
            mergeTargetId = currentStop.id; // Latch the target
            banner.innerHTML = `🎯 Click stop to merge INTO <b>${mergeTargetId}</b>`;
        ...
        if (listEl) {
            ...
            if (sources.length > 0) {
                ...
                sources.forEach(sourceId => {
                    ...
                    li.innerHTML = `<span>Merged: <b>${sourceId}</b></span><button class="unmerge-btn" data-id="${sourceId}">×</button>`;
    ...
    document.getElementById('btn-manual-merge')?.addEventListener('click', () => {
        console.log('[DevTools] Manual Merge Clicked');
        if (!currentStop) {
            console.error('[DevTools] currentStop is null');
            return;
        }
        const input = document.getElementById('manual-merge-id');
        const idToMerge = input.value.trim();
        console.log('[DevTools] Merging:', idToMerge, 'INTO', currentStop.id);

        if (!idToMerge) {
            console.warn('[DevTools] No ID entered');
            return;
        }

        if (String(idToMerge) === String(currentStop.id)) {
            alert("Cannot merge stop into itself.");
            return;
        }

        // Optional: Check if stop exists? Assuming user knows what they are doing or copying ID.
        // But let's be safe and check allStops if possible, or just allow it (force merge).
        // Let's just allow it for now, usually user copies ID.

        // Custom Confirmation UI instead of window.confirm
        const banner = document.getElementById('picking-mode-banner');
        console.log('[DevTools] Banner element:', banner);
        ...
        if (banner) {
            banner.innerHTML = `
                <div style="pointer-events: auto; display:flex; align-items:center; justify-content:center; gap:10px">
                     <span>Merge <b>${idToMerge}</b> INTO <b>${currentStop.id}</b>?</span>
                     <button id="btn-confirm-yes-manual" class="action-btn" style="padding:4px 12px; height:auto; font-size:12px;">Yes</button>
                     <button id="btn-confirm-no-manual" class="action-btn btn-outline" style="padding:4px 12px; height:auto; font-size:12px; background:white; color:black;">Cancel</button>
                </div>
             `;
            banner.style.display = 'block';

            // Hide panel locally to show banner clearly (optional, but consistent with picking mode)
            // panel.style.display = 'none'; 

            document.getElementById('btn-confirm-yes-manual').onclick = (ev) => {
                ev.stopPropagation();
                merges[idToMerge] = currentStop.id;
                window.stopsConfig.merges = merges;
                input.value = ''; // Clear
            ...
            document.getElementById('btn-confirm-no-manual').onclick = (ev) => {
                ev.stopPropagation();
                // Cancelled
                banner.style.display = 'none';
                // panel.style.display = 'block';
    ...
    map.on('click', (e) => {
        console.log('[DevTools] Global Map Click. PickMode:', isPicking, 'Target:', mergeTargetId);

        if (!isPicking || !mergeTargetId) return;
        ...
        const pickedId = feature.properties.id;
        console.log('[DevTools] Picked Feature ID:', pickedId);

        if (String(pickedId) === String(mergeTargetId)) {
            alert("Cannot merge stop into itself.");
            return;
        }
        ...
        if (banner) {
            banner.innerHTML = `
                <div style="pointer-events: auto; display:flex; align-items:center; justify-content:center; gap:10px">
                     <span>Merge <b>${pickedId}</b> INTO <b>${mergeTargetId}</b>?</span>
            ...
            document.getElementById('btn-confirm-yes').onclick = (ev) => {
                ...
                // Force UI to show the TARGET again
                window.selectDevStop(merges[pickedId]);
                showStatus('Merged! Click Save.');
            };
        ...
        fetch('/api/save-stops-config', {
            ...
            .then(res => {
                if (res.ok) {
                    ...
                    setTimeout(() => showStatus('Reload to see changes.'), 2000);
                } else {
                    return res.text().then(text => { throw new Error(text); });
                }
            })
            .catch(e => {
                alert('Save failed: ' + e.message + '. Server restart required if 404.');
